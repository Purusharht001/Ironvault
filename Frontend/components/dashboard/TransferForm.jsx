...
export function TransferForm({ accountNumber, balanceCents, }) {
    ...
    const validateForm = () => {
        ...
        if (formData.toAccountNumber.length !== VALIDATION.ACCOUNT_NUMBER_LENGTH) {
            setError(`Account number must be ${VALIDATION.ACCOUNT_NUMBER_LENGTH} digits`);
            return false;
        }
        ...
        const amountCents = Math.round(parseFloat(formData.amountDollars) * FORMATS.CENT_DIVISOR);
    ...
    const balanceDollars = (balanceCents / FORMATS.CENT_DIVISOR).toFixed(FORMATS.DECIMAL_PLACES);
    return (<form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <Input id="toAccountNumber" name="toAccountNumber" type="text" placeholder="Enter 12-digit account number" value={formData.toAccountNumber} onChange={handleChange} disabled={isLoading} maxLength={VALIDATION.ACCOUNT_NUMBER_LENGTH}/>
      ...
      <div>
        ...
        <Input id="amountDollars" name="amountDollars" type="number" placeholder="0.00" step="0.01" min="0" value={formData.amountDollars} onChange={handleChange} disabled={isLoading}/>
        <p className="text-xs text-muted-foreground mt-1">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <Button type="button" variant="outline" size="sm" onClick={generateRandomReferenceId} disabled={isLoading}>
            Generate Random
          </Button>
        </div>
        <Input id="referenceId" name="referenceId" type="text" placeholder="e.g., transfer-001-abc123" value={formData.referenceId} onChange={handleChange} disabled={isLoading}/>
        <p className="text-xs text-muted-foreground mt-1">
      ...
      {/* Error Message */}
      {error && (<div className="bg-destructive/10 border border-destructive/20 rounded p-4">
