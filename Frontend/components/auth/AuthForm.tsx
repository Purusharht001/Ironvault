...
export function AuthForm({ type }: AuthFormProps) {
  ...
  const getPasswordStrength = (password: string): {
    ...
    if (password.length < 12) return { level: 'fair', color: 'bg-orange-500' };
  ...
  const passwordStrength =
    type === 'signup' ? getPasswordStrength(formData.password) : null;
  ...
  const validateForm = (): boolean => {
    if (type === 'signin') {
      ...
      if (
        formData.username.length < VALIDATION.MIN_USERNAME_LENGTH ||
        ...
        {type === 'signup' && formData.password && (
          <div className="mt-2">
            <div className="flex items-center gap-2">
              <div
                className={`h-1 flex-1 rounded-full ${passwordStrength?.color}`}
              />
